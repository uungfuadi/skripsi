...
from .models import Product, Kategori, Kontak,  Slide, Inspirasi
...
class CheckoutView(View):
    ...
    def post(self, request):
        ...
        bukti_transfer = request.POST.get ('bukti_transfer')
        ...
        if metode_pembayaran=="":
               messages.error(request, 'metode pembayaran Masih kosong')
               context['has_error'] = True
        ...
        for r in keranjang:
            ...
            dibeliupdate.stok -= int(r['quantity'])
            dibeliupdate.save()
...
def inspirasi(request):
    inspirasi_list = Inspirasi.objects.all()
    return render(request, 'inspirasi.html', {'inspirasi_list': inspirasi_list})

def deskripsi_inspirasi(request, inspirasi_id):
    inspirasi = get_object_or_404(Inspirasi, id=inspirasi_id)
    return render(request, 'deskripsi_inspirasi.html', {'inspirasi': inspirasi})
